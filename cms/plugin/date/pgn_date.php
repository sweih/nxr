<?
	...
	class pgnDate extends Plugin {
		...
		function edit(&$form) {
			...
			$form->add(new DateInput($lang->get("date", "Date"), "pgn_date", "VALUE", $condition, "width:350,size:255", ""));				
		...
		function preview() {
			global $lang;
			$content = getDBCell("pgn_date", "VALUE", "FKID = $this->fkid");
		...
		function registration() {
			...
			if ($auth->checkPermission("ADMINISTRATOR")) {
				...
				$this->description = "Date-input Plugin";
				...
				$this->installHandler->addDBAction("CREATE TABLE `pgn_date` (`FKID` bigint(20) NOT NULL default '0', `VALUE` DATETIME NULL,  PRIMARY KEY  (`FKID`),  UNIQUE KEY `FKID` (`FKID`)) TYPE=MyISAM;");

				// SQL for deleting the tables from the database. 
				$this->uninstallHandler->addDBAction("DROP TABLE `pgn_date`");
