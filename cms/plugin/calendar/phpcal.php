...
{ //class in session
	...
	function phpCal($month, $year)
	{
		...
		$this->setStyleVars();
		$this->setStyles();
		$this->setPassThruVars();
	}
	
	/************************************************/
	//function setPassThruVars() will set the variables, that will be automatically be attached to the links. Enter names only, they will be get by search and value.
	function setPassThruVars($vars = "") {
		if ($vars=="") $vars = array("page", "v");
		$this->passThruVars = "";
		for ($i=0; $i<count($vars); $i++) {
			...
			if (isset($$vars[$i])) {
				...
				$value = value($vars[$i], "NOSPACES","");
			}
			$this->passThruVars.= "&".$vars[$i]."=".$value;
		}
	}
	...
	function createCal()
	{
		...
		$cal .= sprintf("<td bgcolor=\"%s\" align=\"center\" onclick=\"window.location.href='%s?month=%s&year=%s%s';\" onmouseover=\"style.backgroundColor='%s'\" onmouseout=\"style.backgroundColor='%s'\"%s><h1%s>&lt;</h1></td>\n", $this->normalDayRollOverColor, $this->calendarLink, $this->prevMonth, $this->prevYear, $this->passThruVars, $this->curDayColor, $this->normalDayRollOverColor, $this->borderStyleTdOne, $this->fontStyleOne);
		...
		$cal .= sprintf("<td bgcolor=\"%s\" align=\"center\" onclick=\"window.location.href='%s?month=%s&year=%s%s';\" onmouseover=\"style.backgroundColor='%s'\" onmouseout=\"style.backgroundColor='%s'\"%s><h1%s>&gt;</h1></td>\n", $this->normalDayRollOverColor, $this->calendarLink, $this->nextMonth, $this->nextYear, $this->passThruVars, $this->curDayColor, $this->normalDayRollOverColor, $this->borderStyleTdThree, $this->fontStyleOne);
		$cal .= sprintf("</tr>\n");
		$cal .= sprintf("<tr>\n");
		...
		if (strlen($day) > 0){
			...
			if (in_array($dateCheck, $this->eventDays)){
				if (($day == date("j")) && ($this->curMonth == date("m")) && ($this->curYear == date("Y"))){
					$cell = sprintf("<td align=\"center\" bgcolor=\"%s\" onclick=\"window.location.href='%s?month=%s&day=%s&year=%s%s';\" onmouseover=\"style.backgroundColor='%s'\" onmouseout=\"style.backgroundColor='%s'\"%s><h1%s><b>%s</b></h1></td>\n", $this->eventDayColor, $this->dayLink, $this->curMonth, $day, $this->curYear, $this->passThruVars, $this->eventDayRollOverColor, $this->eventDayColor, $borderStyle, $this->fontStyleTwo, $day);
					} else {
					$cell = sprintf("<td align=\"center\" bgcolor=\"%s\" onclick=\"window.location.href='%s?month=%s&day=%s&year=%s%s';\" onmouseover=\"style.backgroundColor='%s'\" onmouseout=\"style.backgroundColor='%s'\"%s><h1%s>%s</h1></td>\n", $this->eventDayColor, $this->dayLink, $this->curMonth, $day, $this->curYear, $this->passThruVars, $this->eventDayRollOverColor, $this->eventDayColor, $borderStyle, $this->fontStyleTwo, $day);
					}
				} else {
				if (($day == date("j")) && ($this->curMonth == date("m")) && ($this->curYear == date("Y"))){
					$cell = sprintf("<td align=\"center\" bgcolor=\"%s\" onclick=\"window.location.href='%s?month=%s&day=%s&year=%s%s';\" onmouseover=\"style.backgroundColor='%s'\" onmouseout=\"style.backgroundColor='%s'\"%s><h1%s><b>%s</b></h1></td>\n", $this->curDayColor, $this->dayLink, $this->curMonth, $day, $this->curYear, $this->passThruVars, $this->curDayRollOverColor, $this->curDayColor, $borderStyle, $this->fontStyleTwo, $day);
					} else {
					$cell = sprintf("<td align=\"center\" bgcolor=\"%s\" onclick=\"window.location.href='%s?month=%s&day=%s&year=%s%s';\" onmouseover=\"style.backgroundColor='%s'\" onmouseout=\"style.backgroundColor='%s'\"%s><h1%s>%s</h1></td>\n", $this->normalDayColor, $this->dayLink, $this->curMonth, $day, $this->curYear, $this->passThruVars, $this->normalDayRollOverColor, $this->normalDayColor, $borderStyle, $this->fontStyleTwo, $day);
					}
				}
			} else {
