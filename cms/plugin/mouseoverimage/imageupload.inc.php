...
*/
	 ...
	 * @package DatabaseConnectedInput
	 */
	class ImageUpload3 extends WUIInterface {
		var $v_label;
		...
		function ImageUpload3($label, $fieldname, $file_id, &$pgnref) {
			...
			$this->label = $label;
			$this->fkid = $file_id;
			$this->name = "image" . $fieldname .$specialID;
			...
			$this->v_wuiobject->additionalAttribute = "onChange='preview".$fieldname.$this->fkid."();'";
			$this->id = $fieldname.$specialID;
		}
		...
		function process() {
			global $c;
				...
				if ($this->suffix != "") {
					...
					$width = $size[0];
					$height = $size[1];
					$oldfilename = getDBCell("pgn_mouseoverimage", $this->fieldname, "FKID = " . $this->fileId);
					$filename = $this->fileId .$this->fieldname. "." . $this->suffix;
					// updating the database
					addUpdate("pgn_mouseoverimage", $this->fieldname, $filename, "FKID = " . $this->fileId, "TEXT");
					addUpdate("pgn_mouseoverimage", "WIDTH", $width, "FKID = " . $this->fileId, "NUMBER");
					addUpdate("pgn_mouseoverimage", "HEIGHT", $height, "FKID = " . $this->fileId, "NUMBER");
					...
					// updating the database
					addUpdate("pgn_mouseoverimage", $this->fieldname, $this->filename, "FKID = " . $this->fileId, "TEXT");
					addUpdate("pgn_mouseoverimage", "WIDTH", $width, "FKID = " . $this->fileId, "NUMBER");
					addUpdate("pgn_mouseoverimage", "HEIGHT", $height, "FKID = " . $this->fileId, "NUMBER");					
		...
		function draw() {
			...
			if ($this->fkid != "") {
				$currentimage = $c["devfilesdocroot"].getDBCell("pgn_mouseoverimage", $this->fieldname, "FKID = ".$this->fkid);
				$filename = getDBCell("pgn_mouseoverimage", $this->fieldname, "FKID = ".$this->fkid);
			}
			if ($currentimage == $c["devfilesdocroot"])
				$currentimage = $c["host"].$c["docroot"]."img/ptrans.gif";
			echo "<tr><td colspan=\"2\" class=\"standardlight\" align=\"center\"><img id=\"prv".$this->fieldname.$this->fkid."\" src=\"".$currentimage."\"></td></tr>";
			echo '<script language="JavaScript">';
			echo '  function preview'.$this->fieldname.$this->fkid.'() {';
			echo '    var target = document.form1.'.$this->name.'.value;';
			echo '	  if (target == "") target = "'.$c["devfilesdocroot"].$filename.'";';			
			echo '    document.getElementById("prv'.$this->fieldname.$this->fkid.'").src = target;';
			echo ' }';
			echo '</script>';
			return 2;
		}
