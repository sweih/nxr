<?
	...
	class pgnGoogleCoordinates extends Plugin {
		...
		/**
		  ...
		  */
		function edit(&$form) {
			global $lang, $sid, $c, $page;
			include_once "coords_input.php";		
	   		
			// add button for external editor.
			$condition = "FKID = $this->fkid";
			$form->add(new CoordinatesInput($lang->get("sel_coords", "Please choose the coordinates of a location."), $this->management_table, 'LAT', 'LNG', $condition));			
		...
		function preview() {
			global $lang;
			$content = 'Latitude: ' . getDBCell($this->management_table, "LAT", "FKID = $this->fkid").'<br>';
			$content.= 'Longitude: ' . getDBCell($this->management_table, "LNG", "FKID = $this->fkid");
		...
		/**
		   ...
		   */
		function draw($param = "") {
				$content = array(	getDBCell($this->management_table, "LAT", "FKID = $this->fkid"), 
													getDBCell($this->management_table, "LNG", "FKID = $this->fkid") );
			return $content;
		}
		...
		function createRecord() {
			...
			$createHandler->addDBAction("INSERT INTO $this->management_table ($this->pk_name, LNG, LAT) VALUES ($this->fkid, 0,0)");
		...
		function createVersion($newid) {
			...
			$querySQL = "SELECT LNG, LAT FROM $this->management_table WHERE $this->pk_name = $this->fkid";
			...
			$sql = "INSERT INTO $this->management_table ($this->pk_name, LAT, LNG) VALUES ($newid, $lat, $lng)";
			return $sql;
		}
		...
		function registration() {
			...
			if ($auth->checkPermission("ADMINISTRATOR")) {
				...
				$this->name = "Google Coordinates";
				// A short description, what the Plugin is for.
				$this->description = "Google-Coordinates Plugin";
				...
				$this->installHandler->addDBAction("CREATE TABLE `pgn_googlecoordinates` (`FKID` bigint(20) NOT NULL default '0', `LNG` DOUBLE NULL, `LAT` DOUBLE NULL,  PRIMARY KEY  (`FKID`),  UNIQUE KEY `FKID` (`FKID`)) TYPE=MyISAM;");

				// SQL for deleting the tables from the database. 
				$this->uninstallHandler->addDBAction("DROP TABLE `pgn_googlecoordinates`");
