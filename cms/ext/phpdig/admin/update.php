...
if (is_array($list_exclude)) {
    ...
    foreach ($list_exclude as $this_exclude) {
         ...
         print "<a href='update.php?site_id=$site_id&ex_id=$ex_id&sid=".$sid."' target='_self'><img src='no.gif' width='10' height='10' border='0' align='middle' alt='".phpdigMsg('delete')."'></a>&nbsp;<font COLOR='red'>$ex_path</font><br/>\n";
    }
}
?>
...
for ($n = 0; $n<$num; $n++) {
    ...
    if (!$locked) {
        if ($path_name) {
           print "<a href='update.php?path=".urlencode($path_name)."&amp;site_id=$site_id&amp;sid=$sid&amp;deny=1&amp;sup=1#$aname2' target='_self' ><img src='deny.gif' width='10' height='10' border='0' align='middle' alt='".phpdigMsg('exclude')."' /></a>&nbsp;\n";
        }
        else {
...
<a href="index.php?sid=<?php echo $sid;?>" target='contentset'>[<?php phpdigPrnMsg('back') ?>]</a> <?php phpdigPrnMsg('to_admin') ?>.
</body>
</html>