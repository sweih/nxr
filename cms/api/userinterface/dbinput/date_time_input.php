<?
	...
	/**
	* Draws a to the database-connected Date and Time - Inputbox.
	* @todo Do Check, whether browser is js-enhanced or not an provide
	* alternatively JS-Input functions!
	* @package DatabaseConnectedInput
	*/
	class DateTimeInput extends DBO {
		...
		function check() {
			...
			if ($page_state == "processing") {
				...
				if (strlen($this->value) < 16 && strlen($this->value) > 0) {
					$this->addError("DATEFORMAT", $lang->get("DATEFORMAT"));
				} else if (strlen($this->value) >= 16) {
					if (substr($this->value, 4, 1) != "-" || substr($this->value, 7, 1) != "-" || substr($this->value, 10, 1) != " " || substr($this->value, 13, 1) != ":") {
						$this->addError("DATEFORMAT", $lang->get("DATEFORMAT"));
					} else {
						$year = substr($this->value, 0, 4);
						$month = substr($this->value, 5, 2);
						$day = substr($this->value, 8, 2);
						$hour = substr($this->value, 11, 2);
						$minute = substr($this->value, 14, 2);
