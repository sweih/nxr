<?php
	 ...
	 * so you can implement your own hashing-function. If you do not
	 * want the script-key to be hashed, here you can change this
	 */
	...
	function jpcache_varkey() {
		$varkey = "POST=" . serialize($_POST). " GET=" . serialize($_GET);
	...
	function jpcache_init() {
		...
		header ("X-Cache: jpcache v" . $GLOBALS["JPCACHE_VERSION"] . " - " . $GLOBALS["JPCACHE_TYPE"]);
	}
	...
	function jpcache_gc() {
		...
		if ($GLOBALS["JPCACHE_GC"] > 0) {
			...
			if (((mt_rand() % $precision) / $precision) <= ($GLOBALS["JPCACHE_GC"] / 100)) {
	...
	function jpcache_start() {
		...
		if (jpcache_check()) {
			...
			print jpcache_flush($GLOBALS["jpcachedata_gzdata"], $GLOBALS["jpcachedata_datasize"], $GLOBALS["jpcachedata_datacrc"]);
