<?php
	 ...
	 */
	function jpcache_db_connect() { $GLOBALS["sql_link"] = @mysql_connect($GLOBALS["JPCACHE_DB_HOST"], $GLOBALS["JPCACHE_DB_USERNAME"], $GLOBALS["JPCACHE_DB_PASSWORD"]); }

	/* jpcache_db_disconnect()
	 *
	 * Closes connection to the database
	 */
	function jpcache_db_disconnect() { mysql_close ($GLOBALS["sql_link"]); }
	...
	function jpcache_db_query($query) {
		...
		$ret = @mysql_db_query($GLOBALS["JPCACHE_DB_DATABASE"], $query, $GLOBALS["sql_link"]);
	...
	function jpcache_restore() {
		$res = jpcache_db_query("select GZDATA, DATASIZE, DATACRC from " . $GLOBALS["JPCACHE_DB_TABLE"] . " where CACHEKEY='" . addslashes($GLOBALS["jpcache_key"]). "' and (CACHEEXPIRATION>" . time(). " or CACHEEXPIRATION=0)");
	...
	function jpcache_write($gzdata, $datasize, $datacrc) {
		...
		$res = jpcache_db_query("select CACHEEXPIRATION from $dbtable" . " where CACHEKEY='" . addslashes($GLOBALS["jpcache_key"]). "'");

		if (!$res || mysql_num_rows($res) < 1) {
			// Key not found, so insert
			$res = jpcache_db_query(
				"insert into $dbtable" . " (CACHEKEY, CACHEEXPIRATION, GZDATA," . " DATASIZE, DATACRC) values ('" . addslashes($GLOBALS["jpcache_key"]). "'," . (($GLOBALS["JPCACHE_TIME"] != 0) ? (time() + $GLOBALS["JPCACHE_TIME"]) : 0) . ",'" . addslashes($gzdata). "', $datasize, $datacrc)");
		...
		// the same.
			...
			// Key found, so update
			$res = jpcache_db_query("update $dbtable set CACHEEXPIRATION=" . (($GLOBALS["JPCACHE_TIME"] != 0) ? (time() + $GLOBALS["JPCACHE_TIME"]) : 0) . ", GZDATA='" . addslashes($gzdata). "', DATASIZE=$datasize, DATACRC=$datacrc where" . " CACHEKEY='" . addslashes($GLOBALS["jpcache_key"]). "'");
	...
	function jpcache_do_gc() {
		jpcache_db_query ("delete from " . $GLOBALS["JPCACHE_DB_TABLE"] . " where CACHEEXPIRATION<=" . time(). " and CACHEEXPIRATION!=0");
