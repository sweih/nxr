	...
	if (!defined("PCLTRACE_LIB")) {
		...
		function TrOn($p_level = 1, $p_mode = "memory", $p_filename = "trace.txt") {
			...
			switch ($p_mode) {
				...
				default: $g_pcl_trace_mode = "logged";
			}
		...
		function TrDisplay() {
			...
			for ($i = 0; $i < sizeof($g_pcl_trace_entries); $i++) {
				...
				switch ($g_pcl_trace_entries[$i][type]) {
					...
					default: echo "<td><font size=2 face=$v_font>" . $g_pcl_trace_entries[$i][name] . "(" . $g_pcl_trace_entries[$i][param] . ")</font></td>";
				}
		...
		function PclTraceAction($p_entry) {
			...
			if ($g_pcl_trace_mode == "normal") {
				for ($i = 0; $i < $p_entry[index]; $i++)echo "---";
