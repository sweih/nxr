...
*/
class phpOpenTracker_API_weekdays extends phpOpenTracker_API_Plugin {
  var $apiCalls = array('weekdays');
  var $apiType = 'get';
  var $whereString = "";
  ...
  */
  function run($parameters) {
    ...
    if ($parameters['what'] == 'avg_vis_length') return $this->_run_avg_time($parameters);   
  ...
  function _run_avg_time($parameters) {
    $result = array();
    for ($i=0; $i<7; $i++) {
    	$this->db->query("SELECT AVG(duration) AS CALC FROM pot_nxlog WHERE weekday = $i ".$this->whereString."");
  ...
  function _run_avg_clickstream($parameters) {
    $result = array();
    for ($i=1; $i<=7; $i++) {
    	$this->db->query("SELECT AVG(pi) AS CALC FROM pot_nxlog WHERE weekday = $i ".$this->whereString."");
    	$row = $this->db->fetchRow();
    	$day = array("timestamp" => $i, "value" => ($row["CALC"] == NULL)? 0:(int)$row["CALC"]);
    	array_push($result, $day);
    } 	
  ...
  function _run_visits($parameters) {
    $result = array();
    for ($i=1; $i<=7; $i++) {
    	$this->db->query("SELECT COUNT( pi ) AS CALC FROM pot_nxlog WHERE weekday = $i ".$this->whereString."");
    	$row = $this->db->fetchRow();
    	$day = array("timestamp" => $i, "value" => ($row["CALC"] == NULL)? 0:(int)$row["CALC"]);
    	array_push($result, $day);
    } 	
  ...
  function _run_pi($parameters) {
    $result = array();
    for ($i=1; $i<=7; $i++) {
    	$this->db->query("SELECT SUM( pi ) AS CALC FROM pot_nxlog WHERE weekday = $i ".$this->whereString."");
    	$row = $this->db->fetchRow();
    	$day = array("timestamp" => $i, "value" => ($row["CALC"] == NULL)? 0:(int)$row["CALC"]);
    	array_push($result, $day);
    } 	
