...
// +---------------------------------------------------------------------+
//
// $Id: index.php,v 1.1 2003/09/01 17:19:32 sven_weih Exp $
//
...
if (isset($start)) {
  ...
  $totalVisitors = phpOpenTracker::get(
    array(
      'client_id' => $clientID,
      'api_call'  => 'visitors',
      'start'     => $start,
...
switch ($page) {
      ...
      foreach ($clientIDs as $clientID) {
        ...
        $client->setVar(
          ...
          array(
            ...
            // Total Visitors for current client
            phpOpenTracker::get(
              array(
                'client_id' => $clientID,
                'api_call'  => 'visitors',
            ...
            // Visitors this month for current client
            phpOpenTracker::get(
              array(
                'client_id' => $clientID,
                'api_call'  => 'visitors',
    ...
    while ($year <= $lastYear) {
      ...
      $itemMonthlyStatistics->setVar(
        ...
        array(
          ...
          $visitors,
          $totalVisitors ? number_format(((100 * $visitors) / $totalVisitors), 2) : 0,
    ...
    for ($i = 0; $i < sizeof($pi); $i++) {
      ...
      $itemDailyStatistics->setVar(
        ...
        array(
          ...
          $visitors[$i]['value'],
          $totalVisitors ? number_format(((100 * $visitors[$i]['value']) / $totalVisitors), 2) : '0'
        )
      );
  ...
  case 'day': {
    ...
    // Query visitors for this client and each day of this day
    $visitors = phpOpenTracker::get(
      array(
        'client_id' => $clientID,
        'api_call'  => 'visitors',
...
$content->setVar(
  ...
  array(
    ...
    isset($totalVisitors) ? $totalVisitors : 0,
