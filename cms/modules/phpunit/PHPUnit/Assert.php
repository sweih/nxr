<?php
	...
	// +------------------------------------------------------------------------+
	//
	// $Id: Assert.php,v 1.2 2003/08/12 23:15:30 sven_weih Exp $
	//
	...
	class PHPUnit_Assert {
		...
		*/
		function assertEquals($expected, $actual, $message = '', $delta = 0) {
			if ((is_array($actual) && is_array($expected)) || (is_object($actual) && is_object($expected))) {
				...
				$message = sprintf('%sexpected %s, actual %s', !empty($message) ? $message . ' ' : '', $expected, $actual);

				if ($actual !== $expected) {
					return $this->fail($message);
				}
			} elseif (is_numeric($actual) && is_numeric($expected)) {
				$message = sprintf('%sexpected %s%s, actual %s', !empty($message) ? $message . ' ' : '', $expected, ($delta != 0) ? ('+/- ' . $delta) : '', $actual);
		...
		*/
		function assertNotNull($object, $message = '') {
			$message = sprintf('%sexpected NOT NULL, actual NULL', !empty($message) ? $message . ' ' : '');
		...
		*/
		function assertNull($object, $message = '') {
			$message = sprintf('%sexpected NULL, actual NOT NULL', !empty($message) ? $message . ' ' : '');
		...
		*/
		function assertSame($expected, $actual, $message = '') {
			$message = sprintf('%sexpected two variables to refer to the same object', !empty($message) ? $message . ' ' : '');
		...
		*/
		function assertNotSame($expected, $actual, $message = '') {
			$message = sprintf('%sexpected two variables to refer to different objects', !empty($message) ? $message . ' ' : '');
		...
		*/
		function assertTrue($condition, $message = '') {
			$message = sprintf('%sexpected true, actual false', !empty($message) ? $message . ' ' : '');
		...
		*/
		function assertFalse($condition, $message = '') {
			$message = sprintf('%sexpected false, actual true', !empty($message) ? $message . ' ' : '');
		...
		* @static
		*/
		function assertType($expected, $actual, $message = '') { return $this->assertEquals($expected, gettype($actual), $message); }
